...
def constructentReplaceComma(te):
    print(te)
    regexStart = re.compile('<entity type=([a-z A-Z]+) uri=null>')
    ...
    while te.find('<entity') >-1:
        ...
        print(startindex)
        print(te)
        etype=re.search(regexStart,te).group(1)
        print('etype '+etype)
        ...
        entitystring+='\"entity\":\"'+etype+'\"}'
        #print(entitystring)
        entities+=entitystring
        if te.find('<entity') >-1:
            entities+=','
    resultstring='\"text\":\"'+te.replace('"',' ').replace('\\','').replace('\n',"")+'\",\"entities\":['+entities+']'
    return resultstring
